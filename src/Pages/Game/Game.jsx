...
import { HandFinger, HeartPlus } from "tabler-icons-react";
...
export default function Game({ data }) {
  ...
  const fetchFavourite = async () => {
    try {
      ...
      response.data.includes(data.appid)
        ? setFavourite(true)
        : setFavourite(false);
    } catch (error) {
  ...
  const handlePlayModalClose = () => {
    ...
    sendTimeToServer(sessionTime / 60);
  };

  return (
    <>
      <Modal
        opened={playModalOpen}
      ...
      <Box>
        ...
        <Grid grow style={{ padding: 20, margin: 5 }}>
          ...
          <Grid.Col md={6} lg={3}>
            ...
            <Space mb={10} />
            <Box>
              {data?.steamspy_tags?.map((c, i) => (
                <Badge key={i} style={{ marginRight: 10 }}>
                  {c}
                </Badge>
              ))}
            </Box>
            ...
            {usertoken ? (
              <Group position="center">
                <Button
                  color={favourite ? "red" : "green"}
                  onClick={() => handleFav(data.appid)}
                >
                  <HeartPlus style={{ marginRight: 10 }} />
                  {favourite ? "Remove From Favourite" : "Add To Favourite"}
                </Button>
                <Button color={"green"} onClick={() => setPlayModalOpen(true)}>
                  <HandFinger style={{ marginRight: 10 }} />
                  Vote Now
                </Button>
              </Group>
            ) : (
              <Center style={{ fontWeight: 500, color: "red" }}>
                Sign-in To Vote
              </Center>
            )}
          </Grid.Col>
